...
test("rollback CLI root-establishment failure performs no downstream reads or writes", () => {
  const fixture = mkdtempSync(resolve(tmpdir(), "cadence-rollback-invalid-root-"));
...
test("rollback programmatic API is independent of caller cwd and INIT_CWD", () => {
  const unrelated = mkdtempSync(resolve(tmpdir(), "cadence-rollback-api-cwd-"));
  const moduleUrl = pathToFileURL(resolve(__dirname, "vs005-rollback.ts")).href;
  const script = `(async()=>{const {rollbackVs005Application}=await import(${JSON.stringify(moduleUrl)});try{await rollbackVs005Application({request:{},currentDeploymentEvidence:{},targetDeploymentEvidence:{},currentConfig:{},targetPolicy:{},provider:{},verify:async()=>({})});console.log("UNEXPECTED_SUCCESS")}catch{console.log("REJECTED")}})()`;
  try {
