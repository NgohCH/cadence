...
if (
  ...
  throw new Error(
    "Expected Web environment must be local, qa, or beta."
  );
}
...
const projectId =
  process.env.VITE_PROJECT_ID?.trim() ?? "";

const projectRef =
  process.env.VITE_SUPABASE_PROJECT_REF?.trim() ?? "";
...
if (
  ...
  throw new Error(
    "When configured, Beta VITE_PROJECT_ID must be a UUID."
  );
}
...
if (!projectRef) {
  throw new Error(
    `${expectedEnvironment.toUpperCase()} Web mode requires VITE_SUPABASE_PROJECT_REF.`
  );
}
...
if (
  ...
  throw new Error(
    `${expectedEnvironment.toUpperCase()} Web Supabase URL does not match VITE_SUPABASE_PROJECT_REF.`
  );
}


const localHostedApi =
  parsedApiUrl.protocol === "http:" &&
  (
    parsedApiUrl.hostname === "127.0.0.1" ||
    parsedApiUrl.hostname === "localhost"
...
if (
  ...
  throw new Error(
    "QA/Beta Web mode requires either local Cadence API port 3000 or an HTTPS API endpoint."
  );
}


console.log(
  `Cadence Web environment preflight passed: ${expectedEnvironment} -> API ${parsedApiUrl.host}, Supabase ${parsedSupabaseUrl.host}${projectId ? `, Project ${projectId}` : ", Project not yet provisioned"}`
);
